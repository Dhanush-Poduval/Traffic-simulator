...
export default function IntersectionC() {
  ...
  const fetchDetails=async()=>{
    try{
      const res=await fetch("http://127.0.0.1:8000/details/Signal_C")
      ...
      setChart(prev => {
        const updated = [
          ...prev,
          { time: new Date().toLocaleTimeString(), cars: data.car_amount }
        ];
  ...
  useEffect(() => {
   ...
   const signal_red=async()=>{
      try{
        await fetch('http://127.0.0.1:8000/signal/Signal_C/red',{
          method:'POST',
        })
    ...
    const signal_green=async()=>{
      try{
        await fetch('http://127.0.0.1:8000/signal/Signal_C/green',{
          method:'POST',
        })
      ...
      if(!intervalRef.current){
        intervalRef.current=setInterval(async()=>{
         await fetch("http://127.0.0.1:8000/intersection/Signal_C",{
          method:'POST'
         })
         await fetchDetails();
          const res= await fetch("http://127.0.0.1:8000/details/Signal_C")
