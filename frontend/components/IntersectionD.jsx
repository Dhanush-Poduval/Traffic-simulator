...
export default function IntersectionD() {
  ...
  const fetchDetails=async()=>{
    try{
      const res=await fetch("http://127.0.0.1:8000/details/Signal_D")
  ...
  useEffect(() => {
   ...
   const signal_red=async()=>{
      try{
        await fetch('http://127.0.0.1:8000/signal/Signal_D/red',{
          method:'POST',
        })
    ...
    const signal_green=async()=>{
      try{
        await fetch('http://127.0.0.1:8000/signal/Signal_D/green',{
          method:'POST',
        })
      ...
      if(!intervalRef.current){
        intervalRef.current=setInterval(async()=>{
         await fetch("http://127.0.0.1:8000/intersection/Signal_D",{
          method:'POST'
         })
         await fetchDetails();
          const res= await fetch("http://127.0.0.1:8000/details/Signal_D")
